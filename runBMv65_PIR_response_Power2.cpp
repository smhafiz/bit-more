...
{
  int ntrials = 1, ntrials_pruned = 1, nmetrics = 4, confidence = 2;
  ...
  FILE *f = fopen("power2_pir_response_Jan26.txt", "w");
  int err;

  AES_KEY aeskey;
  AES_set_encrypt_key(_mm_set_epi64x(597349, 121379), &aeskey);

  constexpr std::array<uint8_t, 4> Lognbytes_per_row = {22, 22, 20, 18};
  
  for_constexpr<for_bounds<0, 6>, for_bounds<0, 4>>(
      [&](auto il, auto ir)
  {
    constexpr uint8_t lognitems = 8;
    constexpr size_t nitems = (1ULL << lognitems);
    constexpr uint8_t lognbytes_per_row = std::get<ir.value>(Lognbytes_per_row);
    constexpr size_t nbytes_per_row = (1ULL << lognbytes_per_row);
    ...
    constexpr std::array<uint8_t, 6> Nservers = {128, 64, 32, 16, 8, 4};
    ...
    fprintf(f,"***********************************\nEach row: %.3f MiB, Database: %.5f GiB, In memory: %.5f GiB.\n", (float)nbytes_per_row/(1024*1024), (float)(nitems*nbytes_per_row)/(1024*1024*1024),(float)(alloc_size)/(1024*1024*1024));    
    ...
    for(int trials=0;trials<ntrials;++trials)
    {
      ...
      for (size_t i = 0; i < nwords_per_row; ++i)
      {
        if(result[i].m256[0][0]!=0x00) {correct_ness = false; }
      }
      if (correct_ness) printf("%u, %u[%d]-> c\n",nservers, lognbytes_per_row, trials);
      else printf("%u, %u[%d]->Incorrect!\n",nservers,lognbytes_per_row,trials);
