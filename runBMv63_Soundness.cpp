...
int main(int argc, char * argv[])
{
  ...
  char* metrics[] = {"All query generation (us):\t", "Query expansion per server (ms):", "Response generation per server (ms):", "Record reconstruction (ns):\t"};
  struct timeval tvalBefore, tvalAfter;
  FILE *f = fopen("soundness_pir_Jan28.txt", "w");
  int err;
      ...
      
  {
    ...
    constexpr uint16_t soundness = std::get<is.value>(Soundness);
    ...
    fprintf(f,"\t\trows: 2^%u, ell: %u, soundness: %u\n", lognitems, nservers, soundness);
    for(int trials=0;trials<ntrials;++trials)
    {
      ...
      if (correct_ness) printf("%u[%d]->Correct protocol!\n",soundness, trials);
