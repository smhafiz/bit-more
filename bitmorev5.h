    ...
    if constexpr(std::exp2(nkeys) != static_cast<double>(nservers))
    {
      ...
      while (!mask.all())
      {
        ...
        for (size_t i = 0; i < nkeys / 64; ++i)
        {
          ...
          (rnd <<= 64) |= rbuf;
          modulo = (((modulo << 64*(nkeys/64 - i)) % nservers) + (rbuf % nservers)) % nservers;
        }
        if constexpr(remainder)
        {
          ...
          (rnd <<= remainder) |= rbuf;
          modulo = (((modulo << remainder) % nservers) + (rbuf % nservers)) % nservers;
        }

        if (!mask.test(modulo)) { queries[modulo] = permutation ^ rnd; mask.set(modulo); }
      }
    }
    else
    {
