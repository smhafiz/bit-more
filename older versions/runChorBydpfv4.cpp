...
{
  using namespace dpf;
  int ntrials = 10, ntrials_pruned = 5, nmetrics = 4, confidence = 2;
  ...
  constexpr std::array<uint8_t, 6> Lognitems = {20, 18, 16, 14, 12, 10};//22, 20, 18, 16, 14, 12, 
      ...
      [&](auto iw, auto ir)
  {
    ...
    for(int trials=0;trials<ntrials;++trials)
    {
      ...
      time_elapses[0][trials] = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
      ...
      time_elapses[1][trials] = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
      ...
      time_elapses[2][trials] = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    for (int i = 0; i < nmetrics; ++i)
    {
      for(int tt=0;tt<ntrials;++tt){
        sum_for_sd[i]+=(time_elapses[i][tt]-means[i])*(time_elapses[i][tt]-means[i]);//pow(abs(time_elapses[i][tt]-means[i]),2);
      }
      s_ds[i] = std::sqrt(sum_for_sd[i]/(ntrials-1));
    }
    for(int ii=0;ii<nmetrics;++ii)
    {
      fprintf(f1,"%s\tmean: %.5f\tsd: %.5f\tsupport: %i\n", metrics[ii], means[ii], s_ds[ii], ntrials);
    }
    ...
    for(int i = 0; i < nmetrics; ++i)
    {
      for(int j=0;j<ntrials;++j)
      {
        if(time_elapses[i][j]<=prune_upper[i] && time_elapses[i][j]>=prune_lower[i] && support[i] < ntrials_pruned)
        {
          ...
          fprintf(f1,"(%d) %f\n", i+1, time_elapses_pruned[i][support[i]]);
          support[i]++;
        }
